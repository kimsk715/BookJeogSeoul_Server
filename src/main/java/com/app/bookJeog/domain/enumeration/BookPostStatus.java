...
public enum BookPostStatus {
    ...
    private BookPostStatus(String code){
        this.code = code;
    }
...
//    직접 작성한 code 혹은 label이 있을 경우 재정의해야한다.
    @JsonCreator
    public static BookPostStatus forCode(String code){
//        화면에서 전달받은 문자열(code)을
        for(BookPostStatus bookPostStatus  : BookPostStatus.values()){
//            전체 code와 비교하여 같다면
            if(bookPostStatus.getCode().equals(code)){
