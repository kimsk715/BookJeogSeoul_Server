...
public enum BookPostReportStatus {
    WAITING("처리대기"),
    DONE("처리완료");
    ...
    private BookPostReportStatus(String code){
        this.code = code;
    }
...
//    직접 작성한 code 혹은 label이 있을 경우 재정의해야한다.
    @JsonCreator
    public static BookPostReportStatus forCode(String code){
//        화면에서 전달받은 문자열(code)을
        for(BookPostReportStatus bookPostReportStatus  : BookPostReportStatus.values()){
//            전체 code와 비교하여 같다면
            if(bookPostReportStatus.getCode().equals(code)){
//                해당 code의 memberType 객체를 리턴한다.
                return bookPostReportStatus;
            }
        }
