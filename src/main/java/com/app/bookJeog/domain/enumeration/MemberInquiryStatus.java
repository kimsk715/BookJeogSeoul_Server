...
public enum MemberInquiryStatus {
    WAITING("처리대기"),
    DONE("처리완료");
    ...
    private MemberInquiryStatus(String code){
        this.code = code;
    }
...
//    직접 작성한 code 혹은 label이 있을 경우 재정의해야한다.
    @JsonCreator
    public static MemberInquiryStatus forCode(String code){
//        화면에서 전달받은 문자열(code)을
        for(MemberInquiryStatus memberInquiryStatus  : MemberInquiryStatus.values()){
//            전체 code와 비교하여 같다면
            if(memberInquiryStatus.getCode().equals(code)){
//                해당 code의 memberType 객체를 리턴한다.
                return memberInquiryStatus;
            }
        }
