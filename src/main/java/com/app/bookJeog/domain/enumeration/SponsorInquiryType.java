...
public enum SponsorInquiryType {
    BOOK("도서관련 문의"),
    ACCOUNT("계정관련 문의"),
    ETC("기타문의");
    ...
    private SponsorInquiryType(String code){
        this.code = code;
    }
...
//    직접 작성한 code 혹은 label이 있을 경우 재정의해야한다.
    @JsonCreator
    public static SponsorInquiryType forCode(String code){
//        화면에서 전달받은 문자열(code)을
        for(SponsorInquiryType sponsorInquiryType : SponsorInquiryType.values()){
//            전체 code와 비교하여 같다면
            if(sponsorInquiryType.getCode().equals(code)){
//                해당 code의 memberType 객체를 리턴한다.
                return sponsorInquiryType;
            }
        }
